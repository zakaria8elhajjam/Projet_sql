            ...
            if (max_inscription($database, $idPersonne)) {
                ...
                if ($_POST['isTest'] == "soutien") {
                    if (searchmatiere($database, $idPersonne, $_POST['matiere'], $_POST['niveau']) == false) {
                        ...
                        mysqli_query($database, $insertAppartenance);
                        datedebut($database, $idGroupe, $idPersonne);
                        $sql = "SELECT COUNT(`NomFormation`) AS countf FROM `Suivie` WHERE `IdPersonne`='{$idPersonne}' AND NomFormation='{$_POST['isTest']}'"; // tous les formation dont l'etudiant est inscrit
                        ...
                        header("Location:refus.php?refus=soutien");
                        exit();
                    }
                }
            } 
                ...
                $insertAppartenance = "INSERT INTO `Appartenance` (`IdPersonne`, `IdGrp`, `NbrAbscence`, `NbrSeance`) VALUES ('{$newid}', '{$idGroupe}', '0', '{$_POST['nbrseance']}')";
                mysqli_query($database, $insertAppartenance);
...
<body>
    ...
    <form class='form' method="post">
        ...
        <select name="niveau" required>
            <option value="niveau" selected disabled>Choisissez le niveau</option>
        ...
        <select name="matiere" required>
            <option value="matiere" selected disabled>Choisissez la matière</option>
        ...
        <input type="radio" name="isTest" value="soutien"><label>Soutien</label><br>
